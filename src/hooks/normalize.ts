...
const SEARCH_TOOLS = new Set(["WebSearch", "SearchWeb"]);
const FETCH_TOOLS = new Set(["WebFetch", "FetchURL"]);
...
export function normalize(raw: any, phase: "pre" | "post"): NormalizedInput | null {
  ...
  const toolUseId = raw.tool_use_id ?? raw.tool_call_id ?? "";
  const sessionId = raw.session_id ?? "";
  const cwd = raw.cwd ?? "";

  const result: NormalizedInput = { toolUseId, sessionId, cwd, type, query };
  if (phase === "post") {
    result.results = raw.tool_response ?? raw.tool_output ?? null;
  }
  return result;
}
